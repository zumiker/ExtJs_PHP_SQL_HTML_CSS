...
$sql = "SELECT DISTINCT
			FAC, CONGROUP, ABI_NUMBER, LASTNAME, FIRSTNAME, PATRONYMIC, SEX, TO_CHAR(BIRTHDAY, 'dd.mm.yyyy') as BIRTHDAY, PASNUMBER,
			HOST, FACID, CON_ID, NABOR, MUSOR, PRIORITET, TCENAME, EDUDOCUMENT, DOCUMENT, EDUDOCUMENTNUMBER, DOC,
			SIROTAID, ABITUR, AWARD, TAWNAME, DATE_IN, ABI_ID, NN, PHONE, PHONE2, EDUFINISH, POSTINDEX, ADDRESS,
			STAID, STANAME
		FROM ABIVIEW_AMBAR
		WHERE CON_ID='$groid'
			$wher1e
...
$html .= "<tr bgcolor='$col'>" .
	...
	if ( !$TAWNAME && !$ABITUR )
		...
		$sqlpr="SELECT DISTINCT a.ID_PR, a.PREDMET, BALL100, r.ID_PR as ID_PR1
				FROM ABI_MINI_EXAM a, ABIVIEW_RASPISANIE r
				WHERE ABI_ID='$ABI_ID'
					AND CON_ID='$CON_ID'
					AND a.ID_PR=r.ID_PR
				ORDER BY a.ID_PR";
...
$html .= "<td align='center'>$tmp</td>";
		$sqlpr2="SELECT predmet
				FROM ABI_NA_TRAD tr, abi_predmet pr
				WHERE ABI_ID='$ABI_ID'
					AND  tr.id_predmet = pr.id_pr
				ORDER BY pr.ID_PR";
...
//					"<tr bgcolor='$col'><td colspan='8' align='center' valign='middle' height='12'>$POSTINDEX $ADDRESS</td>" .
					"<tr bgcolor='$col'><td colspan='8' align='center' valign='middle' height='12'>$ADDRESS</td>" .
...
//	$sqlpodval="SELECT * FROM ABI_MINI_PODVAL WHERE facid='$divid' AND CON_ID='$groid' AND SPECID='$subid'";//CON_ID = '$id_con' AND SPECID = '$id_spec'";
	$sqlpodval="SELECT distinct *
				FROM ABI_MINI_PODVAL
				WHERE CON_ID='$groid'";
