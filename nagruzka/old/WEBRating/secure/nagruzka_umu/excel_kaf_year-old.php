...
$worksheet ->write($n,1, '',$titleFormat1); 
$worksheet ->write($n,2, 'преподавателей кафедры '.$DIVABBREVIATE,$titleFormat1); 
$worksheet ->write($n,3, '',$titleFormat1); 
$worksheet ->write($n,4, '',$titleFormat1); 
$worksheet ->write($n,5, '',$titleFormat1); 
$worksheet ->write($n,6, '',$titleFormat1); 
...
{	
	...
	$n+=2;
	$worksheet ->write($n,0, 'Объем выполненной работы за  '.$god.' учебный год',$titleFormat1); 
	$worksheet ->write($n,1, '',$titleFormat1); 
		...
		for ($i=0;$i<ora_numrows($cur);$i++)
				{
					...
					$worksheet->writeString($n+1,6, trim($F6), $format_bold);
					$worksheet->writeString($n+1,7, trim($F_ITOGO7), $format_grey_bold);
					$worksheet->writeString($n+1,8, trim($F8), $format_bold);
