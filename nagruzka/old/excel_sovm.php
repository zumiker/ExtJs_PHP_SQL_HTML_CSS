...
$worksheet =& $workbook->addWorksheet('Совм.');
...
$worksheet ->write($n,0, 'Совместители',$titleFormat); 
...
"F_VSEGOPLAN, F15, F16, F17, F18, F19, F20, F_VSEGO21, F22, F23, F24, FPRIM, F_VSEGO25, F_SEMESTR, STAT, DOL_SMALL, STAT_ID, STAVKA, PREPID ".
"FROM Z_PREPOD_NAGRUZKA WHERE DIVID='$kaf' AND SPRING_AUTUMN='$sem' AND YEAR_GROCODE='$god' AND STAT_ID = 4". 
...
for ($i=0;$i<ora_numrows($cur);$i++)
		{
			...
			$PREPID=ora_getcolumn($cur,57);
			$FIODOL=$FIO." ".$DOL_SMALL;
			$n=$n+2;
			if($zagod==1)
			{$sql2="select sum(SEMESTR)SEMESTR, 
		   	sum(f_SEMESTR)f_SEMESTR 
		 	from Z_PREPOD_NAGRUZKA 
		 	WHERE  DIVID='$kaf' AND YEAR_GROCODE='$god' AND STAT_ID = 4 AND PREPID='$PREPID'";

					$cur2=ora_do($conn,$sql2);
					$vsegogod=ora_getcolumn($cur2,0); 
					$fvsegogod=ora_getcolumn($cur2,1);
			}  
			
			if($STAT_ID=='2' && $notfirst==0)
			{
				
				$worksheet->writeString($n+7,0, "Совместители", $sheetTitleFormat);
				$notfirst=1;
				$worksheet ->setMerge($n+7,0,$n+7,26);
				$n++;
				

			}
			//if($notfirst==1 && $STAT_ID=='2')
			if($STAVKA!=1)
			{
			...
			if($vsegogod>900)
			{if($zagod==1)$worksheet->writeString($n,27, trim($vsegogod), $format_red);}
			else
			...
			if($zagod==1)$worksheet->writeString($n+1,27, trim($fvsegogod), $format_itog);
			$worksheet ->setMerge($n,0,$n+1,0);
 ...
 for($s=0;$s<=$numzz;$s++)
 {
 	...
 	{
 		case '0':
 		$where = "WHERE DIVID='$kaf' AND SPRING_AUTUMN='$sem' AND YEAR_GROCODE='$god' AND FOR_SORT=2";
 		$name="ИТОГО:";		
 		break;
 		case '1':
 		$where = "WHERE DIVID='$kaf' AND YEAR_GROCODE='$god' AND FOR_SORT=2";
 		$name="ИТОГО за год:"; 			
 		break;
 	}
 		...
 		if($zagod==1)$sql2="select YEAR_GROCODE, sum(SEMESTR)SEMESTR,
		   	sum(f_SEMESTR)f_SEMESTR
		 	from Z_semestr_kategor
		 	WHERE  DIVID='$kaf' AND YEAR_GROCODE='$god' AND FOR_SORT=2
