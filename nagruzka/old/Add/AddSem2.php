	...
	$conn=ora_logon("GANGTEMP@IUSDB.DEVELOPERS","gdb");
		...
		if($manid==0)
		{
		 //echo "удаляю ";
				 ...
				 $sql="SELECT COUNT(*) FROM NAGRUZKA_SEM WHERE NAGID='$nag' AND ID='$semid'";
				$cur=ora_do($conn,$sql);
				...
				{
			       $sql="DELETE FROM NAGRUZKA_SEM WHERE NAGID='$nag'  AND ID='$semid'";
				   $cur=ora_do($conn,$sql);
		...
		else
		{
				...
				$sql="SELECT COUNT(*) FROM NAGRUZKA_SEM WHERE NAGID='$nag' $sss";
				$cur=ora_do($conn,$sql);
				$lec=ora_getcolumn($cur,0);
				
				if($lec==0 || $semid==0)
				{
					$sql="INSERT INTO NAGRUZKA_SEM (NAGID,PREPID_SEM,ROOM_SEM) VALUES('$nag','$manid','$room_sem')";
					$cur=ora_do($conn,$sql);
					echo "Запись сохранена<br/>";
				}
				else
				{
					$sql="UPDATE NAGRUZKA_SEM SET PREPID_SEM='$manid',ROOM_SEM='$room_sem' WHERE NAGID='$nag' $sss";
					//echo $sql;
					$cur=0;
					$cur=ora_do($conn,$sql);
