...
<?php
	...
	function showReport($db_handle)
	{
		...
		<form method="GET" action="master.php">
			...
			echo getSelect('SELECT id, name FROM tdolzhnost WHERE KOL_SIMVOL IS NOT NULL ORDER BY id', $dolzhnost, 'dolzhnost');
			?>
			<br /><br />
			Категория<br />
			<?php
...
/*			$sql = "select 
					   ...
					   TSTEPEN.NAME,
					   TZVANIE.NAME as ZVN,
				       ID_MANID,CHIEF, p.DIVID, p.MANID
				from 
					 MAN,
					 V_SPI_PREPOD_POISK p,
					 TSTEPEN,
					 TDOLZHNOST,
					 TEMPSTATUS,
					 TZVANIE,
				...
				where 
					  p.MANID=MAN.MANID 
					  and TSTEPEN.ID=p.TST_ID 
					  and TDOLZHNOST.ID=p.TDO_ID 
					  and p.TEM_ID = TEMPSTATUS.ID
					  and p.TZV_ID (+)= TZVANIE.ID
					  and p.TRA_ID = TRABSTAVKA.ID
					  and MAN.MANLASTNAME LIKE UPPER('$query%')
					  and DIVISION.DIVID = p.DIVID
				order by MANLASTNAME";*/
//				$sql = "select * from V_SPI_PREPOD_POISK where fio_prepod like upper('$query%')";
/*				$sql = "select Initcap(MANLASTNAME) || ' ' || Initcap(MANFIRSTNAME) || ' ' || Initcap(MANPATRONYMIC), DIVABBREVIATE, MANSEX, TEM, TRA, DOL, NAME, ZVN, ID_MANID, CHIEF, DIVID, MANID 
						from V_SPI_PREPOD_POISK where MANLASTNAME like upper('$query%') ORDER BY MANLASTNAME";*/
				$sql = "select initcap(fio), divabbreviate, mansex, stat, stavka, dol, stepen, zvan, prepod_id, zavkaf, divid, manid 
						from v_spi_prepod
						order by fio";
			...
			if ($curs)
			{
				...
				do
				{
					$fio = ora_getcolumn($curs, 0);
					$manid = ora_getcolumn($curs, 11);
					$id_manid = ora_getcolumn($curs, 8);
					echo '<tr>';
//					if ( $id_manid || ( $manid == $id_manid ) )
					if ( $id_manid )
					{
						echo '<td><a href="master.php?action=show_man&manid=' .  $manid .'&id_manid=' .  $id_manid .'">' . $fio . '</a></td>';
						echo '<td>' . ora_getcolumn($curs, 1) . '</td>';
						echo '<td>' . ora_getcolumn($curs, 3) . '</td>';
						echo '<td><a href="master.php?action=show_man&manid=' .  $manid .'">Добавить категорию</a></td>';
					}
					else
					{
						echo '<td><a href="master.php?action=show_man&manid=' .  $manid .'">' . $fio . '</a></td>';
						echo '<td>' . ora_getcolumn($curs, 1) . '</td>';
						echo '<td>' . ora_getcolumn($curs, 3) . '</td>';
					}
					echo '</tr>';
...
/*		if ($trash)
		...
		$sql = "select Initcap(MANLASTNAME),Initcap(MANFIRSTNAME),Initcap(MANPATRONYMIC),MANSEX,TEM,TRA,DOL,NAME,ZVN,MANID ,CHIEF, DIVID,ID_MANID
from (
select MANLASTNAME,MANFIRSTNAME,MANPATRONYMIC,MANSEX,TEM,TRA,DOL,TST_ID,name as ZVN,MANID ,CHIEF, DIVID,ID_MANID
from (
select MANLASTNAME,MANFIRSTNAME,MANPATRONYMIC,MANSEX,TEMPSTATUS.NAME as TEM,TRABSTAVKA.NAME as TRA,TDOLZHNOST.NAME as DOL,sz.TST_ID,sz.TZV_ID,MAN.MANID ,CHIEF, DIVID,ID_MANID
                from 
                     MAN,PREPOD,
                     TDOLZHNOST,
                     TEMPSTATUS,
...
where m.TST_ID = t.ID(+)
order by MANLASTNAME,MANFIRSTNAME,MANPATRONYMIC";*/
$sql = "select Initcap(fio), MANSEX, stat, stavka, DOL, stepen, ZVaN, manid, zavkaf, DIVID, prepod_id
		from V_SPI_PREPOD
		where DIVID=$divid
		ORDER BY fio";
/*        $sql = "select 
...
/*			$name			= join( array(ora_getcolumn($curs, 0) , ora_getcolumn($curs, 1) , ora_getcolumn($curs, 2)), " ");
			...
			$name			= ora_getcolumn($curs, 0);
			$sex			= ora_getcolumn($curs, 1);
			$kat			= ora_getcolumn($curs, 2);
			$stavka			= ora_getcolumn($curs, 3);
			$dol			= ora_getcolumn($curs, 4);
			$stepen			= ora_getcolumn($curs, 5);			
			$zvan			= ora_getcolumn($curs, 6);
			$manid			= ora_getcolumn($curs, 7);
			$chief			= ora_getcolumn($curs, 8);
			$current_divid	= ora_getcolumn($curs, 9);
			$id_manid		= ora_getcolumn($curs, 10);
			if ($chief)
			{
				$name = "<b>$name</b>";
			}
    ...
    function ShowMan($db_handle)
    {
        ...
        if ($id_manid)
		{
            $curs = ora_do($db_handle, "select FACNAME,lower(DIVNAME) from DIVISION,FACULTY,v_spi_PREPOD 
										where DIVISION.FACID=FACULTY.FACID and v_spi_PREPOD.DIVID=DIVISION.DIVID and prepod_id=$id_manid");
            $facname = ora_getcolumn($curs, 0);
            $divname = ucfirst(ora_getcolumn($curs, 1));
            StartShowResultset("Факультет: <b>$facname</b><br>Кафедра: <b>$divname</b>", false);
            $action = "update_man";
