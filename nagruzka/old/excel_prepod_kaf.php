...
if ($kaf!='0')
 $sql="SELECT FIO_PREPOD,DOL,STEPEN,ZVAN,STAT,STAVKA,ZAVKAF,net FROM V_SPI_PREPOD WHERE DIVID=$kaf ORDER BY FIO_PREPOD";
 else 
 $sql="SELECT FIO_PREPOD,DOL,STEPEN,ZVAN,STAT,STAVKA,ZAVKAF,net FROM V_SPI_PREPOD WHERE FACID=$fac ORDER BY FIO_PREPOD";
...
$cur=ora_do($conn,$sql);
		...
		//$worksheet ->write(j,j, 'Российский Государственный Университет нефти и газа имени И.М. Губкина',$titleFormat1); 
			...
			$worksheet ->write($j+8,7, $net,$format);
