...
$worksheet =& $workbook->addWorksheet('Распред.нагрузки(Осень)');
...
$POTOK_LEK_old='netu';
$conn=ora_logon("GANGTEMP@IUSDB.DEVELOPERS","gdb");
$sql="SELECT PREDMET, GROCODE, LEKTOR, ROOM_LEC, SEMINAR, ROOM_SEM, LABRAB, ROOM_LAB, PRIM, LEC, LAB, SEM, POTOK_LEK ".
	 "FROM Z_DISPETCH_NAGRUZKA WHERE DIVID='$kaf' AND SPRING_AUTUMN='$sem' AND YEAR_GROCODE='$god' ".
	 "ORDER BY POR_SORT, KURS, PREDMET, POTOK_LEK, GROCODE, LEKTOR, SEMINAR, LABRAB";
...
for ($i=0;$i<ora_numrows($cur);$i++)
		{
			...
			$POTOK_LEK=ora_getcolumn($cur,12);
			//if($POTOK_LEK!=$POTOK_LEK_old)
			//$LEKTOR_old='netu';
			//$worksheet->writeString($n,15, trim($POTOK_LEK), $format);
