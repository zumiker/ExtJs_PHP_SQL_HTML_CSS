...
$worksheet ->write($n,1, '',$titleFormat1); 
if($fac==999)
$worksheet ->write($n,2, 'преподавателей университета',$titleFormat1);
else
 $worksheet ->write($n,2, 'преподавателей факультета '.$FACULTY,$titleFormat1);
$worksheet ->write($n,3, '',$titleFormat1); 
$worksheet ->write($n,4, '',$titleFormat1); 
$worksheet ->write($n,5, '',$titleFormat1); 
$worksheet ->write($n,6, '',$titleFormat1); 
...
{	
	...
	if($fac!='999')
	{
		$worksheet ->write($n,28, '',$titleFormat1);
		$worksheet ->write($n,29, '',$titleFormat1);
	}
	...
	if($fac!='999')
		{
			$worksheet ->write($n,28, 'Средняя ауд. нагрузка',$verticalformat);
			$worksheet ->write($n,29, 'Средняя нагрузка общ.',$verticalformat);
		}
	...
	if($fac!='999')
		{
			$worksheet ->write($n,28, '',$verticalformat);
			$worksheet ->write($n,29, '',$verticalformat);
		}
	...
	if($fac!='999')
		{
			$worksheet ->write($n,28, '28',$titleFormat);
			$worksheet ->write($n,29, '29',$titleFormat);
		}
	...
	$worksheet->setColumn(0,5,5);
	$worksheet->setColumn(0,6,4);
	$worksheet->setColumn(0,7,5);
	...
	if($fac!='999')
		{
			$worksheet->setColumn(0,28,7);
			$worksheet->setColumn(0,29,7);
		}
	...
	$worksheet ->setMerge(5,28,7,28);
	$worksheet ->setMerge(5,29,7,29);
	...
	for($j=1;$j<=7;$j++)
	{
		...
		switch($j)
		{
			...
			case '1':
			$where="WHERE YEAR_GROCODE='$god' $FACUL AND FOR_SORT='1' AND GRUPPA='$j' group by $FACUL_GROUP_BY YEAR_GROCODE";
			 break;
			...
			case '5':
			$where="WHERE YEAR_GROCODE='$god' $FACUL AND FOR_SORT='2' group by $FACUL_GROUP_BY YEAR_GROCODE";
			break;
			
			case '6':
			$where="WHERE YEAR_GROCODE='$god' $FACUL AND FOR_SORT='3' group by $FACUL_GROUP_BY YEAR_GROCODE";
			break;
		...
		/*$sql="SELECT DIVID, YEAR_GROCODE, YEAR_GROCODE, sum(LECTIME), sum(SEMTIME), sum(LABTIME), 
 		...
 		$sql="select YEAR_GROCODE, YEAR_GROCODE, sum(LECTIME) LECTIME,
		...
		for ($i=0;$i<ora_numrows($cur);$i++)
				{
					...
					$SEMTIME=ora_getcolumn($cur,4);  
					 ...
					 $F_GOD=ora_getcolumn($cur,54);  
					 $KOL_PREPOD=ora_getcolumn($cur,55);
					 $KOL_STAVKI=ora_getcolumn($cur,56);
					 $SR_AUD = $ITOGO7/$KOL_STAVKI;
					 $SR_OB = $GOD/$KOL_STAVKI;
					 $F_SR_AUD = $F_ITOGO7/$KOL_PREPOD;
					 $F_SR_OB = $F_GOD/$KOL_PREPOD;
					...
					$worksheet->writeString($n,5, trim($VSEGO5), $format_grey);
					$worksheet->writeString($n,6, trim($EKZ_ZACH), $format);
					$worksheet->writeString($n,7, trim($ITOGO7), $format_grey);
					...
					$worksheet->writeString($n,13, trim($VSEGO13), $format_grey);
					$worksheet->writeString($n,14, trim($VSEGOPLAN), $format_itog);
					...
					$worksheet->writeString($n,21, trim($VSEGO21), $format_grey);
					...
					$worksheet->writeString($n,26, trim($VSEGO25), $format_grey);
					$worksheet->writeString($n,27, trim($GOD), $format_itog);
					if($fac!='999' && j!=5)
					{
						$worksheet ->writeString($n,28, round(trim($SR_AUD),2), $format_itog_bold);
						$worksheet ->writeString($n,29, round(trim($SR_OB),2), $format_itog_bold);
					}



					$worksheet->writeString($n+1,0, trim(''), $format_bold);
					$worksheet->writeString($n+1,1, trim($KOL_PREPOD), $format_bold);
					$worksheet->writeString($n+1,2, trim($F_LECTIME), $format_bold);
					$worksheet->writeString($n+1,3, trim($F_SEMTIME), $format_bold);
					$worksheet->writeString($n+1,4, trim($F_LABTIME), $format_bold);
					$worksheet->writeString($n+1,5, trim($F_VSEGO5), $format_grey_bold);
					$worksheet->writeString($n+1,6, trim($F_EKZ_ZACH), $format_bold);
					$worksheet->writeString($n+1,7, trim($F_ITOGO7), $format_grey_bold);
					$worksheet->writeString($n+1,8, trim($F_KPR), $format_bold);
					$worksheet->writeString($n+1,9, trim($F9), $format_bold);
					$worksheet->writeString($n+1,10, trim($F10), $format_bold);
					$worksheet->writeString($n+1,11, trim($F11), $format_bold);
					$worksheet->writeString($n+1,12, trim($F12), $format_bold);
					$worksheet->writeString($n+1,13, trim($F_VSEGO13), $format_grey_bold);
					$worksheet->writeString($n+1,14, trim($F_VSEGOPLAN), $format_itog_bold);
					$worksheet->writeString($n+1,15, trim($F15), $format_bold);
					$worksheet->writeString($n+1,16, trim($F16), $format_bold);
					$worksheet->writeString($n+1,17, trim($F17), $format_bold);
					$worksheet->writeString($n+1,18, trim($F18), $format_bold);
					$worksheet->writeString($n+1,19, trim($F19), $format_bold);
					$worksheet->writeString($n+1,20, trim($F20), $format_bold);
					$worksheet->writeString($n+1,21, trim($F_VSEGO21), $format_grey_bold);
					$worksheet->writeString($n+1,22, trim($F22), $format_bold);
					$worksheet->writeString($n+1,23, trim($F23), $format_bold);
					$worksheet->writeString($n+1,24, trim($F24), $format_bold);
					$worksheet->writeString($n+1,25, trim($PRIM), $format_bold);
					$worksheet->writeString($n+1,26, trim($F_VSEGO25), $format_grey_bold);
					$worksheet->writeString($n+1,27, trim($F_GOD), $format_itog_bold);
					if($fac!='999'  && j!=5)
					{
						$worksheet ->writeString($n+1,28, round(trim($F_SR_AUD),2), $format_itog_bold);
						$worksheet ->writeString($n+1,29, round(trim($F_SR_OB),2), $format_itog_bold);
					}
