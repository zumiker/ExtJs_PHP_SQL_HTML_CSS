...
$sql="SELECT FAC FROM FACULTY WHERE FACID='$fac'";
$cur=ora_do($conn,$sql);
...
for ($j=0;$j<ora_numrows($cur5);$j++)
{
	...
	if ($fac1!=9 && $fac1!=11)	{
		...
		if ($fac!=999)	{
			for ($i=0;$i<ora_numrows($cur);$i++)	{
				...
				$sql3="SELECT D.STAVKA_SOVM, D.VSEGO_SOVM FROM (SELECT STAT_ID,STAT,SUM(SUM_STAVKA) AS STAVKA_SOVM,SUM(VSEGO) AS VSEGO_SOVM from Z_PREPOD_UMU WHERE DIVID='$kaf' AND STAT_ID = 2 group by stat_id,stat) D";
				$cur3=ora_do($conn,$sql3);

				$sql2="select E.STAVKA_SOVM2, E.VSEGO_SOVM2 FROM (SELECT STAT_ID,STAT,SUM(SUM_STAVKA) AS STAVKA_SOVM2,SUM(VSEGO) AS VSEGO_SOVM2 from Z_PREPOD_UMU WHERE DIVID='$kaf' AND STAT_ID = 4 group by stat_id,stat) E";
				$cur2=ora_do($conn,$sql2);

				$sql6="select F.STAVKA_POCH, F.VSEGO_POCH FROM (SELECT STAT_ID,STAT,SUM(SUM_STAVKA) AS STAVKA_POCH,SUM(VSEGO) AS VSEGO_POCH from Z_PREPOD_UMU WHERE DIVID='$kaf' AND STAT_ID = 3 group by stat_id,stat) F";
				...
				///////////////////////////////////////////////////////////////////////////
				$sql0 = "SELECT sum( SUM_STAVKA ) FROM  Z_PREPOD_UMU where DIVID = '$kaf' AND STAT_ID in ( 1, 2 )";
				$cur0 = ora_do($conn,$sql0);
				$stavka_shtat			= ora_getcolumn( $cur0, 0 );

				$sql0 = "SELECT sum( SUM_STAVKA ) FROM  Z_PREPOD_UMU where DIVID = '$kaf' AND STAT_ID in ( 4 )";
				$cur0 = ora_do($conn,$sql0);
				$stavka_sovmest			= ora_getcolumn( $cur0, 0 );

				$sql0 = "SELECT sum( SUM_STAVKA ) FROM  Z_PREPOD_UMU where DIVID   = '$kaf' AND   STAT_ID in ( 1, 2, 4 )";
				$cur0 = ora_do($conn,$sql0);
				$stavka_sht_vsego		= ora_getcolumn( $cur0, 0 );

				$sql0 = "SELECT sum( SUM_STAVKA ) FROM  Z_PREPOD_UMU where DIVID = '$kaf' AND STAT_ID in ( 1, 2, 3, 4 )";
				$cur0 = ora_do($conn,$sql0);
				$stavka_sht_vsego_poch	= ora_getcolumn( $cur0, 0 );
				///////////////////////////////////////////////////////////////////////////
				...
				if ($fac!=999)	{
					...
					$worksheet->writeString($n,2, trim($stavka_shtat).' / '.trim($stavka_sovmest), $format);
					$worksheet->writeString($n,3, trim($stavka_sht_vsego), $format);
					...
					$worksheet->writeString($n,9, $chisl_obsch, $format);
					$worksheet->writeString($n,10,$chisl_aud, $format);
					...
					$worksheet->writeString($k,2, trim($stavka_shtat).' / '.trim($stavka_sovmest), $format);
					$worksheet->writeString($k,3, trim($stavka_sht_vsego_poch), $format);
					...
					$worksheet->writeString($k,9, $chisl_obsch2, $format);
					$worksheet->writeString($k,10,$chisl_aud2, $format);
	...
	//			$itog_3	=$itog_3+$stavka_sht_vsego_poch;
				...
				///////////////////////////////////////////////////////////////////////////
				$sql77="SELECT sum( nvl( PPS, 0 ) ) FROM DIVISION_PPS WHERE DIVID in ( select divid from v_spi_kafedr where facid = '$fac') AND YEAR_GROCODE='$god'";
				$cur77=ora_do($conn,$sql77);
				$itog0=ora_getcolumn($cur77,0);
				
				$sql0 = "SELECT sum( nvl( SUM_STAVKA, 0 ) ) FROM Z_PREPOD_UMU where facid = '$fac' AND STAT_ID in ( 1, 2 )";
				$cur0 = ora_do($conn,$sql0);
				$itog1		= ora_getcolumn( $cur0, 0 );

				$sql0 = "SELECT sum( nvl( SUM_STAVKA, 0 ) ) FROM Z_PREPOD_UMU where facid = '$fac' AND STAT_ID in ( 4 )";
				$cur0 = ora_do($conn,$sql0);
				$itog2= ora_getcolumn( $cur0, 0 );

				$sql0 = "SELECT sum( nvl( SUM_STAVKA, 0 ) ) FROM Z_PREPOD_UMU where facid = '$fac' AND STAT_ID in ( 1, 2, 4 )";
				$cur0 = ora_do($conn,$sql0);
				$itog3= ora_getcolumn( $cur0, 0 );

				$sql0 = "SELECT sum( nvl( SUM_STAVKA, 0 ) ) FROM Z_PREPOD_UMU where facid = '$fac' AND STAT_ID in ( 1, 2, 3, 4 )";
				$cur0 = ora_do($conn,$sql0);
				$itog_3	= ora_getcolumn( $cur0, 0 );
				///////////////////////////////////////////////////////////////////////////
		...
		if ($fac!=999)
		{
			...
			$worksheet->writeString($k,3, trim($itog_3), $format_bold);
			$worksheet->writeString($k,4, trim($itog_4), $format_bold);
			$worksheet->writeString($k,5, trim($itog_5), $format_bold);
			$worksheet->writeString($k,6, trim($itog_6), $format_bold);
			$worksheet->writeString($k,7, trim($itog_7), $format_bold);
			$worksheet->writeString($k,8, '', $format_bold);
			$worksheet->writeString($k,9, round(trim($itog_8)), $format_bold);
			$worksheet->writeString($k,10,round(trim($itog_9)), $format_bold);
