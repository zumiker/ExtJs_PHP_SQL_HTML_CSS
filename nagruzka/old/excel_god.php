...
ora_fetch($cur);
$n=0;
$worksheet ->write($n,0, 'Российский Государственный Университет нефти и газа имени И.М. Губкина',$titleFormat1); 
...
$n++;
/*if($fac==999)
$worksheet ->write($n,0, 'преподавателей университета',$titleFormat1);
else
...
{	
	...
	$worksheet ->write($n,1, 'ППС(в ставках/физ.лицах)',$verticalformat);
		$worksheet ->write($n+1,1, '',$verticalformat);
	$worksheet ->write($n+2,1, '',$verticalformat);  
	$worksheet ->write($n,2, 'НАГРУЗКА ПО УЧЕБНОМУ ПЛАНУ',$titleFormat); 
	...
	$worksheet ->write($n+1,14, '',$verticalformat);
	$worksheet ->write($n+2,14, '',$verticalformat); 
	  
	$worksheet ->write($n,15, 'ДРУГИЕ ВИДЫ УЧЕБНОЙ НАГРУЗКИ',$titleFormat); 
	...
	$worksheet ->write($n,22, 'ПРОЧИЕ ВИДЫ РАБОТ',$titleFormat); 
	...
	$worksheet ->write($n,2, 'АУДИТОРНАЯ НАГРУЗКА',$titleFormat); 
	$worksheet ->write($n,3, '',$titleFormat); 
	$worksheet ->write($n,4, '',$titleFormat); 
	$worksheet ->write($n,5, '',$titleFormat); 
	$worksheet ->write($n,6, '',$titleFormat); 
	...
	$worksheet ->write($n,0, '',$titleFormat); 
	$worksheet ->write($n,1, 'ППС(в ставках/физюлицах)',$verticalformat); 
	...
	$worksheet ->write($n,14, '',$verticalformat); 
	...
	$worksheet->setColumn(0,27,6);
	
	$worksheet ->setMerge(5,0,7,0);
	$worksheet ->setMerge(5,1,7,1);
	$worksheet ->setMerge(5,14,7,14);
	//$worksheet ->setMerge(5,25,7,25);
	//$worksheet ->setMerge(5,26,7,26);
	$worksheet ->setMerge(5,27,7,27);
	
	$worksheet ->setMerge(5,15,6,21);
	
	$worksheet ->setMerge(5,22,6,26);
	...
	for($j=1;$j<=7;$j++)
	{
		...
		switch($j)
		{
			...
			case '1':
			$where="WHERE YEAR_GROCODE='$god' $DIVUL AND FOR_SORT='1' AND GRUPPA='$j' group by DIVID,YEAR_GROCODE";
			 break;
			...
			case '5':
			$where="WHERE YEAR_GROCODE='$god' $DIVUL AND FOR_SORT='2' group by DIVID,YEAR_GROCODE";
			break;
			
			case '6':
			$where="WHERE YEAR_GROCODE='$god' $DIVUL AND FOR_SORT='3' group by DIVID,YEAR_GROCODE";
			break;
			
			case '7':
			$where="WHERE YEAR_GROCODE='$god' $DIVUL group by DIVID,YEAR_GROCODE";
			break;
		}
	/*
		...
		for ($i=0;$i<ora_numrows($cur);$i++)
				{
					...
					$worksheet->writeString($n+1,25, trim($FPRIM), $format_bold);
