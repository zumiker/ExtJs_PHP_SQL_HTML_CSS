...
$worksheet =& $workbook->addWorksheet('Ðàñïðåä.íàãðóçêè');
...
$worksheet ->write($n,0, 'ÐÀÑÏÐÅÄÅËÅÍÈÅ ÍÀÃÐÓÇÊÈ ÍÀ '.$SPRING_AUTUMN.' ÑÅÌÅÑÒÐ '.$god.' Ó×.ÃÎÄÀ',$titleFormat1); 
...
$worksheet ->write($n,1, 'ÄÈÑÖÈÏËÈÍÀ',$titleFormat); 
$worksheet ->write($n+1,1, '',$titleFormat); 
$worksheet ->write($n,2, 'Ïîòîê èëè ãðóïïà',$titleFormat); 
$worksheet ->write($n+1,2, '',$titleFormat); 
$worksheet ->write($n,3, 'ËÅÊÖÈÈ',$titleFormat); 
$worksheet ->write($n,4, '',$titleFormat); 
$worksheet ->write($n,5, '',$titleFormat); 
$worksheet ->write($n+1,3, '÷/í',$titleFormat); 
$worksheet ->write($n+1,4, 'Ô.È.Î. ïðåïîäàâàòåëÿ',$titleFormat); 
$worksheet ->write($n+1,5, '¹ ñïåö. àóä.',$titleFormat); 
$worksheet ->write($n,6, 'ÏÐÀÊÒÈ×ÅÑÊÈÅ ÇÀÍßÒÈß',$titleFormat);
$worksheet ->write($n,7, '',$titleFormat); 
$worksheet ->write($n,8, '',$titleFormat); 
$worksheet ->write($n+1,6, '÷/í',$titleFormat);
$worksheet ->write($n+1,7, 'Ô.È.Î. ïðåïîäàâàòåëÿ',$titleFormat); 
$worksheet ->write($n+1,8, '¹ ñïåö. àóä.',$titleFormat);  
$worksheet ->write($n,9, 'ËÀÁÎÐÀÒÎÐÍÛÅ ÐÀÁÎÒÛ',$titleFormat); 
$worksheet ->write($n,10, '',$titleFormat); 
$worksheet ->write($n,11, '',$titleFormat);
$worksheet ->write($n+1,9,  '÷/í',$titleFormat); 
$worksheet ->write($n+1,10, 'Ô.È.Î. ïðåïîäàâàòåëÿ',$titleFormat);
$worksheet ->write($n+1,11, '¹ ñïåö. àóä.',$titleFormat);
$worksheet ->write($n,12, 'ÏÐÈÌÅ×ÀÍÈÅ',$titleFormat); 
...
for ($i=0;$i<ora_numrows($cur);$i++)
		{
			...
			if ($PREDMET==$PREDMET_old)
			{
				...
				if ($LEKTOR==$LEKTOR_old)
				{
					...
					if ($SEMINAR==$SEMINAR_old)
					{
						...
						$worksheet->writeString($n,10, trim($LABRAB), $format);
					...
					{	
						...
						$worksheet->writeString($n,1, trim($empty), $format);
						//$worksheet->writeString($n,2, trim($GROCODE), $format);
						$worksheet->writeString($n,7, trim($SEMINAR), $format);
				...
				$worksheet->writeString($n,0, trim($empty), $format);
				$worksheet->writeString($n,1, trim($empty), $format);
				$worksheet->writeString($n,4, trim($LEKTOR), $format);
			...
			$worksheet->writeString($n,1, trim($PREDMET), $format);
			$worksheet->writeString($n,2, trim($GROCODE), $format);
			$worksheet->writeString($n,3, trim($LEC), $format);
			$worksheet->writeString($n,4, trim($LEKTOR), $format);
...
$footerFormat->setAlign('center'); 
			...
			$n+=2;
			$worksheet->writeString($n,8, trim($zav_kaf), $footerFormat);
			$worksheet->writeString($n,2, trim($hdate), $footerFormat);
			$n+=10;
