...
$worksheet =& $workbook->addWorksheet('Почас.');
...
$worksheet ->setMerge($n,0,$n+1,0);
	...
	$worksheet ->setMerge($n,26,$n+2,26);
	$worksheet ->setMerge($n,27,$n+2,27);
...
$worksheet ->write($n+1,14, 'ДРУГИЕ ВИДЫ УЧЕБНОЙ НАГРУЗКИ',$titleFormat);  
...
$worksheet ->write($n,0, 'Почасовики',$titleFormat); 
...
"FROM Z_PREPOD_NAGRUZKA WHERE DIVID='$kaf' AND SPRING_AUTUMN='$sem' AND YEAR_GROCODE='$god' AND STAT_ID ='3' ". 
...
for ($i=0;$i<ora_numrows($cur);$i++)
 ...
 for($s=0;$s<=$numzz;$s++)
 {
 	...
 	{
 		case '0':
 		$where = "WHERE DIVID='$kaf' AND SPRING_AUTUMN='$sem' AND YEAR_GROCODE='$god' AND FOR_SORT=3";
 		$name="ИТОГО:";		
 		break;
 		case '1':
 		$where = "WHERE DIVID='$kaf' AND YEAR_GROCODE='$god' AND FOR_SORT=3";
 		$name="ИТОГО за год:";
