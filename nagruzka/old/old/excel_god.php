		...
		for ($i=0;$i<ora_numrows($cur);$i++)
				{
					...
					$worksheet->writeString($n+1,2, trim($F_LECTIME), $format);
					$worksheet->writeString($n+1,3, trim($F_SEMTIME), $format);
					$worksheet->writeString($n+1,4, trim($F_LABTIME), $format);
					$worksheet->writeString($n+1,5, trim($F_VSEGO5), $format);
					$worksheet->writeString($n+1,6, trim($F_EKZ_ZACH), $format);
					$worksheet->writeString($n+1,7, trim($F_ITOGO7), $format);
					$worksheet->writeString($n+1,8, trim($F_KPR), $format);
					...
					$worksheet->writeString($n+1,25, trim($FPRIM), $format);
