...
$worksheet =& $workbook->addWorksheet('Итоговая');
...
$n++;
$worksheet ->write($n,0, 'Учебная нагрузка преподавателей факультета '.$FACULTY,$titleFormat1); 
$worksheet ->write($n,1, '',$titleFormat1); 
$worksheet ->write($n,2, '',$titleFormat1); 
$worksheet ->write($n,3, '',$titleFormat1); 
$worksheet ->write($n,4, '',$titleFormat1); 
...
{	
	...
	if($zagod==1)$worksheet->setColumn(0,27,5);
	/*
	...
	for($j=1;$j<=5;$j++)
	{
		...
		switch($j)
		{
			...
			$where="WHERE SPRING_AUTUMN='$sem' AND YEAR_GROCODE='$god' $FACUL AND FOR_SORT='$j'
			 group  by  DIVID,DIVABBREVIATE,SPRING_AUTUMN,YEAR_GROCODE";
			...
			$where="WHERE YEAR_GROCODE='$god' $FACUL 
		 	group by DIVID,DIVABBREVIATE,YEAR_GROCODE";
		 	$SPRING_AUTUMN="DIVID,";
		 	if($zagod==1)$sql2="select YEAR_GROCODE, sum(SEMESTR)SEMESTR,
		   	sum(f_SEMESTR)f_SEMESTR
		 	from Z_semestr_kategor
		 ...
		 $sql="select DIVID,$SPRING_AUTUMN YEAR_GROCODE, 
		 ...
		 sum( f_vsego21)  f_vsego21,
		  ...
		  sum(f_SEMESTR)f_SEMESTR,   DIVABBREVIATE
		 from Z_semestr_kategor
		 $where";
	//	echo $where;
		$cur=ora_do($conn,$sql);
		for ($i=0;$i<ora_numrows($cur);$i++)
				{
					...
					$fvsegozagod=0;
					 ...
					 $F_SEMESTR=ora_getcolumn($cur,53);  
					...
					$worksheet->writeString($n,4, trim($VSEGO5), $format);
					$worksheet->writeString($n,5, trim($EKZ_ZACH), $format);
					$worksheet->writeString($n,6, trim($ITOGO7), $format);
					...
					$worksheet->writeString($n,12, trim($VSEGO13), $format);
					$worksheet->writeString($n,13, trim($VSEGOPLAN), $format);
					...
					$worksheet->writeString($n,20, trim($VSEGO21), $format);
					...
					$worksheet->writeString($n,25, trim($VSEGO25), $format);
					$worksheet->writeString($n,26, trim($SEMESTR), $format);
					if($zagod==1)$worksheet->writeString($n,27, trim($vsegogod), $format);
			
					$worksheet->writeString($n+1,0, trim(''), $format);
					$worksheet->writeString($n+1,1, trim($F2), $format);
					$worksheet->writeString($n+1,2, trim($F3), $format);
					$worksheet->writeString($n+1,3, trim($F4), $format);
					$worksheet->writeString($n+1,4, trim($F_VSEGO5), $format);
					$worksheet->writeString($n+1,5, trim($F6), $format);
					$worksheet->writeString($n+1,6, trim($F_ITOGO7), $format);
					$worksheet->writeString($n+1,7, trim($F8), $format);
					$worksheet->writeString($n+1,8, trim($F9), $format);
					$worksheet->writeString($n+1,9, trim($F10), $format);
					$worksheet->writeString($n+1,10, trim($F11), $format);
					$worksheet->writeString($n+1,11, trim($F12), $format);
					$worksheet->writeString($n+1,12, trim($F_VSEGO13), $format);
					$worksheet->writeString($n+1,13, trim($F_VSEGOPLAN), $format);
					$worksheet->writeString($n+1,14, trim($F15), $format);
					$worksheet->writeString($n+1,15, trim($F16), $format);
					$worksheet->writeString($n+1,16, trim($F17), $format);
					$worksheet->writeString($n+1,17, trim($F18), $format);
					$worksheet->writeString($n+1,18, trim($F19), $format);
					$worksheet->writeString($n+1,19, trim($F20), $format);
					$worksheet->writeString($n+1,20, trim($F_VSEGO21), $format);
					$worksheet->writeString($n+1,21, trim($F22), $format);
					$worksheet->writeString($n+1,22, trim($F23), $format);
					$worksheet->writeString($n+1,23, trim($F24), $format);
					$worksheet->writeString($n+1,24, trim($PRIM), $format);
					$worksheet->writeString($n+1,25, trim($F_VSEGO25), $format);
					$worksheet->writeString($n+1,26, trim($F_SEMESTR), $format);
					if($zagod==1)$worksheet->writeString($n+1,27, trim($fvsegogod), $format);
					$worksheet ->setMerge($n,0,$n+1,0);
					$n=$n+2;
					ora_fetch($cur);	
					}	
