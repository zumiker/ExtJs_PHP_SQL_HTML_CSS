<?php
/*
шапки к ПДФ и надо добавить колонки общежитие и слово реком.
слева, над " мест в общежитии "
добавть про места в группе и сколько осталось, ооже самое про общагу
*/
...
//$worksheet ->write($n,1, '',$titleFormat1); 
if ($nab == '5'){
	$worksheet ->write($n,1, 'Протокол по зачислению на 1 курс РГУ нефти и газа имени И.М.Губкина ',$format_bold);
}else{
...
//	$n++;
	...
	if($nab == '5')
	{
		$sql="SELECT ABI_NUMBER, LASTNAME, FIRSTNAME, PATRONYMIC, OJID, TAWNAME_SMALL, KATEGORIA, DOC, BALL, KATEGOR" .
		" FROM ABIVIEW_PODVAL_24 " .
		" WHERE CON_ID='$id_con' AND KATEGOR IN (21,23,32) and tur_fix = $tur ORDER BY KATEGOR,BALL DESC, LASTNAME";	
	}
...
for ($j=0;$j<=0;$j++)
		{
			...
			" FROM ABIVIEW_PODVAL_24 " .
			" WHERE  CON_ID='$id_con' AND (TNABOR='$nab')  AND KATEGOR IN (29,31)  AND DOC='Подл' and tur_fix = $tur ORDER BY KATEGOR, BALL desc, LASTNAME";
			}
			else
			{
			$sql="SELECT ABI_NUMBER, LASTNAME, FIRSTNAME, PATRONYMIC, OJID, TAWNAME_SMALL, KATEGORIA, DOC, BALL, KATEGOR, COLOR, PRIORITET" .
			" FROM ABIVIEW_PODVAL_24 " .
			" WHERE  CON_ID='$id_con' AND (TNABOR='$nab')  AND KATEGOR IN (29,31) and tur_fix = $tur ORDER BY KATEGOR, BALL desc, LASTNAME";
			}
			}
			else
			{
