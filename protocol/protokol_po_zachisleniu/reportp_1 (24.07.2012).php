...
$html.= "<tr><td><tr><td><tr><td colspan='5' size='15' align='left'>" .
	...
	if ($nab!='3')
	{
		...
		if($tur==2)
			{
				$html.= "<td><td><tr><td colspan='5' size='15' align='left'>" .
				"Осталось мест в общежитии</td><td  width='15' > $OSTATKI_HOST</td></tr>";	
			}
	}
	else
	{
...
if ($podl!='1' && $tur!='3')
{
	...
	if ($podl==1)
		...
		$s1.=
						"<tr><td rowspan='2' align='center' width='9'><b>№<br> п/п</b></td>" .
...
//		"<td align='center'>$PRIKAZ</td>";
		...
		$sql="SELECT ABI_NUMBER, LASTNAME, FIRSTNAME, PATRONYMIC, HOST, TAWNAME_SMALL, KATEGORIA, DOC, decode(PRIORITET,1,1,4,2,7,3), BALL, PRIKAZ, OJID
        FROM ABIVIEW_PODVAL_24
        WHERE CON_ID='$id_con' AND (TNABOR=3) AND KATEGOR IN (21,23) AND DOC='Подл' and tur_fix = $tur ORDER BY KATEGOR,BALL DESC, LASTNAME";
		}
		else
		{
		$sql="SELECT ABI_NUMBER, LASTNAME, FIRSTNAME, PATRONYMIC, HOST, TAWNAME_SMALL, KATEGORIA, DOC, decode(PRIORITET,1,1,4,2,7,3), BALL, PRIKAZ, OJID
        FROM ABIVIEW_PODVAL_24
        WHERE CON_ID='$id_con' AND (TNABOR=3) AND KATEGOR IN (21,23) and tur_fix = $tur ORDER BY KATEGOR,BALL DESC, LASTNAME";
		}
...
//////////////////////////////////////////////// end	
		for ($j=0;$j<=0;$j++)
		{
			...
			" FROM ABIVIEW_PODVAL_24 " .
			" WHERE  CON_ID='$id_con' AND (TNABOR='$nab') AND KATEGOR='29' AND DOC='Подл' and tur_fix = $tur ORDER BY KATEGOR, BALL desc, LASTNAME";
			}
			else
			{
			$sql="SELECT ABI_NUMBER, LASTNAME, FIRSTNAME, PATRONYMIC, HOST, TAWNAME_SMALL, KATEGORIA, DOC, BALL, KATEGOR, COLOR, decode(PRIORITET,1,1,4,2,7,3), PRIKAZ, OJID " .
			" FROM ABIVIEW_PODVAL_24 " .
			" WHERE  CON_ID='$id_con' AND (TNABOR='$nab') AND KATEGOR='29' and tur_fix = $tur ORDER BY KATEGOR, BALL desc, LASTNAME";
			}
			}
			else
			{
		...
		$s1.=
						"<tr><td rowspan='2' align='center' width='10'><b>№<br> п/п</b></td>" .
						"<td rowspan='2' align='center' width='30'><b>ФИО</b></td>" .
						//"<td align='center' valign='middle'  rowspan=2 ><b>Имя</b></td>" .
						//"<td align='center' valign='middle'  rowspan=2 ><b>Отчество</b></td>" .
						"<td rowspan='2' align='center' width='10'><b>Сумма<br>баллов</b></td>".
