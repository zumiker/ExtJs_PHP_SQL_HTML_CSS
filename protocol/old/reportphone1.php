...
$sql="SELECT SPCNAME, SPCBRIFE, SPCCODENEW FROM ABIVIEW_CON_SPEC WHERE  ID_CON='$id_con' ORDER BY SPCBRIFE";
$cur=ora_do($conn,$sql);
$date = date("d.m.y H:i");
$html.= "<table border='0' width='180'>
			<tr>
				<td colspan='99' family='TimesNewRomanPS-BoldMT' size=17 align='center'>Протокол по зачислению на 1 курс РГУ нефти и газа </td>
			</tr>
			<tr>
				<td colspan='99' family='TimesNewRomanPS-BoldMT' size=17 align='center'>имени И.М.Губкина на $date</td>
			</tr>
			<tr>
				<td colspan='99' family='TimesNewRomanPS-BoldMT' size=17 align='center'>на следующие специальности</td>
			</tr>
			<tr>
				<td colspan='99' align='left'>$CONGROUP   ($name $doc) тур $tur</td>
			</tr>";
for ($i=0;$i<ora_numrows($cur);$i++){
	...
	if ($CONGROUP=='Конкурсная группа 13'){
		if ($nab==1){
			if ($SPCBRIFE!=''){
				$html.= "<tr>
							<td colspan='6' size='14' align='left'>$SPCBRIFE    $SPCNAME</td>
						</tr>";	
...
if ($nab!='3'){
	if ($podl==1)
		$sql="SELECT ABI_NUMBER, LASTNAME, FIRSTNAME, PATRONYMIC, HOST, TAWNAME_SMALL, KATEGORIA, DOC, decode(PRIORITET,1,1,4,2,7,3), BALL, PRIKAZ, OJID, PHONE, PHONE2 
				FROM ABIVIEW_PODVAL_24 
				WHERE CON_ID='$id_con' AND (TNABOR='$nab') AND KATEGOR IN (21,23) AND DOC='Подл' 
				ORDER BY KATEGOR,BALL DESC, LASTNAME";
	else
		$sql="SELECT ABI_NUMBER, LASTNAME, FIRSTNAME, PATRONYMIC, HOST, TAWNAME_SMALL, KATEGORIA, DOC, decode(PRIORITET,1,1,4,2,7,3), BALL, PRIKAZ, OJID, PHONE, PHONE2 
				FROM ABIVIEW_PODVAL_24 
				WHERE CON_ID='$id_con' AND (TNABOR='$nab') AND KATEGOR IN (21,23) 
				ORDER BY KATEGOR,BALL DESC, LASTNAME";
	$cur=ora_do($conn,$sql);
	$num=0;	
	$KATEGORIA_OLD='ned';
	for ($i=0;$i<ora_numrows($cur);$i++){			
		...
		$PHONE = ora_getcolumnnbsp($cur,12);
		$PHONE2 = ora_getcolumnnbsp($cur,13);

		$s1 = "<table border='1' style='text-align:center; font-family: Verdana; font-size: 7px;'>
					<tr>
						<td colspan='99' align=center family='TimesNewRomanPSMT' size='7'><b>СПИСОК К ЗАЧИСЛЕНИЮ</b></td>
					</tr>
					<tr>
						<td rowspan='2' align='center' width='5'><b>№<br> п/п</b></td>
						<td rowspan='2' align='center' width='25'><b>ФИО</b></td>
						<td rowspan='2' class='verticaltext'  align='center' width='32'><b>Телефоны</b></td>
						<td rowspan='2' align='center' width='10'><b>Сумма<br>баллов</b></td>
						<td rowspan='2' align='center' width='5'><b>Приоритет</b></td>
						<td rowspan='2' align='center' width='11'><b>Медаль</b></td>
						<td rowspan='2' align='center'><b>Документ</b></td>
					</tr>
					<tr>
						<td colspan='2' align='center' width='10'><b>Общежитие</b></td>
						<td rowspan='2' align='center' width='10' ><b>№ л.д.</b></td>
						<tr><td align='center'  width='5'><b>Треб.</b></td>
						<td align='center'  width='5'><b>Выдел.</b></td>
					</tr>";
		if($KATEGORIA_OLD!=$KATEGORIA){
			if ($KATEGORIA_OLD=='ned')
				$html.=$s1;
			$html.= "<tr>
						<td colspan='99' family='TimesNewRomanPS-BoldMT' size='12' align='left'>$KATEGORIA</td>
					</tr>";
		...
		$html .= "<TABLE border='1'>
					<tr bgcolor='$col'>
						...
						<td align='center' >$PHONE<br/>$PHONE2</td>
...
"font-family: " .
"Verdana; " .
"font-size: 7px;'><tr><td colspan='99' align=center family='TimesNewRomanPSMT' size='7'><b>СПИСОК К ЗАЧИСЛЕНИЮ</td></b></tr>";
...
"<td colspan='2' align='center' width='10'><b>Общежитие</b></td></tr>".
"<td rowspan='2' align='center' width='10' ><b>№ л.д.</b></td></tr>".
"<tr><td align='center'  width='5'><b>Треб.</b></td>".
"<td align='center'  width='5'><b>Выдел.</b></td></tr>";
...
if ($podl==1)
{
$sql="SELECT ABI_NUMBER, LASTNAME, FIRSTNAME, PATRONYMIC, HOST, TAWNAME_SMALL, KATEGORIA, DOC, BALL, KATEGOR, COLOR, decode(PRIORITET,1,1,4,2,7,3), PRIKAZ, OJID, PHONE, PHONE2 " .
" FROM ABIVIEW_PODVAL_24 " .
...
$PHONE=ora_getcolumnnbsp($cur,14);
$PHONE2=ora_getcolumnnbsp($cur,15);
...
"<td align='center'>$DOC</td>".
"<td align='center'>$HOST</td>";
$html .= "<td align='center' size='9'>$OJID</td>";
if ( $PRIORITET != 1 )
