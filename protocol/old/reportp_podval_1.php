...
if ($podl!='1' && $tur!='3')
{
	...
	/*$html.= "<tr><td></td><td colspan='4' size='15'  align='left'>" .
		...
		$s1.=
						...
						"<td align='center' width='110'><b>ФИО</b></td>" .
						"<td align='center'><b>Балл</b></td>" .
						"<td align='center'><b>Аттестат</b></td>" .
						"<td align='center'><b>Общежитие</b></td></tr>" ;
...
$sql="SELECT ABI_NUMBER, LASTNAME, FIRSTNAME, PATRONYMIC, OJID, TAWNAME_SMALL, KATEGORIA, DOC, BALL, KATEGOR" .
		" FROM ABIVIEW_PODVAL_24 " .
		" WHERE $sql1  $sql3 AND KATEGOR IN (21,23) AND DOC='Подл' ORDER BY KATEGOR,BALL DESC, LASTNAME";
...
/*				$sql="SELECT ABI_NUMBER, LASTNAME, FIRSTNAME, PATRONYMIC, HOST, TAWNAME_SMALL, KATEGORIA, DOC, BALL, KATEGOR, COLOR, PRIORITET, PRIKAZ " .
				" FROM ABIVIEW_PODVAL_24 " .
				" WHERE  $sql1  $sql3   AND KATEGOR IN (29,31) ORDER BY KATEGOR, BALL desc, LASTNAME";*/
/*				$sql="SELECT ABI_NUMBER, LASTNAME, FIRSTNAME, PATRONYMIC FROM ABIVIEW_PODVAL_24 " .
				" WHERE  $sql1  $sql3   AND KATEGOR IN (29,31) ORDER BY KATEGOR, BALL desc, LASTNAME";*/
				$sql="SELECT ABI_NUMBER, LASTNAME, FIRSTNAME, PATRONYMIC, OJID, TAWNAME_SMALL, KATEGORIA, DOC, BALL, KATEGOR" .
						" FROM ABIVIEW_PODVAL_24 " .
						" WHERE $sql1  $sql3 AND KATEGOR IN (29,31) AND DOC='Подл' ORDER BY KATEGOR,BALL DESC, LASTNAME";
			}
			else
			{
			...
			$html.=$s1;
				...
				switch($KATEGOR)
				{
					...
					case 31:
						...
						if ($tur!='3')
						{
							$html.= "<tr><td><tr><td colspan='99' family='TimesNewRomanPS-BoldMT' size=14 align='left'>Выдержавшие вступительные испытания и претендующие на поступление";
							if ($tur=='1')
								$html.= "<br>на 1-й курс университета на вакантные места после 1 тура </td></tr>"; 
							else
								$html.= "<br>на 1-й курс университета на вакантные места после 2 тура </td></tr>"; 
						}
