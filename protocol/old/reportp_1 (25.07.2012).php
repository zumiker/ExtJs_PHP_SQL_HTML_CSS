...
<tr>
	<td colspan='9' align=center family='TimesNewRomanPSMT' size='7'><b>СПИСОК К ЗАЧИСЛЕНИЮ</b></td>
</tr>
<tr>
	...
	if($j==0){
		if ($podl==1){
			$sql="SELECT ABI_NUMBER, LASTNAME, FIRSTNAME, PATRONYMIC, HOST, TAWNAME_SMALL, KATEGORIA, DOC, BALL, KATEGOR, COLOR, decode(PRIORITET,1,1,4,2,7,3), PRIKAZ, OJID 
					FROM ABIVIEW_PODVAL_24 
					WHERE  CON_ID='$id_con' AND (TNABOR='$nab') AND KATEGOR='29' AND DOC='Подл' and tur_fix = $tur 
					ORDER BY KATEGOR, BALL desc, LASTNAME";
		}
		else{
			$sql="SELECT ABI_NUMBER, LASTNAME, FIRSTNAME, PATRONYMIC, HOST, TAWNAME_SMALL, KATEGORIA, DOC, BALL, KATEGOR, COLOR, decode(PRIORITET,1,1,4,2,7,3), PRIKAZ, OJID 
		...
		if ($nab=='3'){
			if($KATEGOR!=$KATEGOR_OLD){
				...
				switch($KATEGOR){
					...
					case 31:
						$html.= "<tr>
									<td colspan='99' family='TimesNewRomanPS-BoldMT' size=12 align='left'>Резерв</td>
								</tr>";
						break;
				}
			}
